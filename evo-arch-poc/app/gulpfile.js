...
gulp.task('build', function(done){
  sequence('clean', ['pages', 'sass', 'js:core', 'js:app', 'images', 'copy'], 'build:notify', done);
});
...
gulp.task('watch', function(){
  ...
  gulp.watch(config.src.layouts,  function(){sequence(['pages:all'], 'reload', 'pages:notify')});
  gulp.watch(config.src.data,     function(){sequence(['pages:all'], 'reload', 'pages:notify')});
  gulp.watch(config.src.scss,     function(){sequence(['sass'], 'reload', 'sass:notify')});
  gulp.watch(config.src.js,       function(){sequence(['js:core', 'js:app'], 'reload', 'js:notify')});
  gulp.watch(config.src.images,   function(){sequence(['images'], 'reload', 'images:notify')});
  gulp.watch(config.src.assets,   function(){sequence(['copy'])});
});
